...
function stringToArray(str) {
    // Розбиваємо рядок на слова, видаляючи пробіли
    let wordsArray = str.split(' ').filter(word => word !== '');
    // Сортуємо масив слів за алфавітним порядком
    wordsArray.sort((a, b) => a.localeCompare(b, 'uk', { sensitivity: 'base' }));
    return wordsArray;
}
...
function removeDuplicatesAndSort(arr) {
    const newArray = [...new Set(arr)]; // Створюємо масив з оригінальних елементів
    newArray.sort((a, b) => a - b); // Відсортовуємо за зростанням
    return newArray;
}
...
function arrayOfEvens(arr) {
    const evenNumbers = arr.filter(number => number % 2 === 0); // Фільтруємо парні числа
    return evenNumbers;
}
